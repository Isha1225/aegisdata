...
// 6 — Live demo
{
  ...
  const steps = [["ANL", "Open Why P0: risk 94/100, factors, Critical Exposure Policy v1.2. Confirm classification. The Task Decision Engine creates the task with four SLA clocks."], ["ANL → OWN", "Assign owner with reason and scope. Status: Awaiting Acceptance, store still Unowned. Nobody is accountable until acceptance."], ["OWN", "Accept ownership. The acknowledge clock stops. Record containment; the 30-minute clock stops. Resolution keeps running."], ["OWN", "Pause with reason, next action, review date: the SLA treatment is shown, never silent. Delegate: executor changes, accountability stays."], ["OWN → Verifier", "Submit for verification. No Mark Complete exists. One seeded task fails verification and reopens itself. The P0 passes and closes."], ["ANL / CISO", "Advance the demo clock on an unacknowledged task: At Risk → SLA Breached → escalated to the analyst, then the Security Director. The timeline shows every actor and time."]];
